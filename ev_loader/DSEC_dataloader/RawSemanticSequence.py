...
class RawSemanticSequence(Sequence):
    """
    ...
    def __getitem__(self, index):
        ...
        semantic_path = self.semantic_label_paths[index]
...
if __name__ == "__main__":
    ...
    for data in seq:
        ...
        segmentation = data["semantic_gt"].numpy()
        frame = torch.zeros((440, 640, 3), dtype=torch.uint8).numpy()
        rendered_events = events.render(frame)
        plt.imshow(rendered_events)
        plt.imshow(segmentation, alpha=0.5)  # Overlay segmentation with some transparency
